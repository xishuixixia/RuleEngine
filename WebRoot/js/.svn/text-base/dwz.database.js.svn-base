...
(function($){
	...
	$.extend({
		...
		bringBack: function(args){
			...
			var temp = DWZ.jsonEval(DWZ.jsonEval(args));
			if(temp.type == 'condition1'){//添加规则中的属性
				$("#dialogBackground").hide();
				$("#content").textbox().insertText("$" + temp.orgName + "$");
			}if(temp.type == 'condition0'){//修改规则
				$("#dialogBackground").hide();
				$("#content0").textbox().insertText("$" + temp.orgName + "$");
			}else if(temp.type.substring(0,10) == 'condition2'){//动作中选择实体属性
				var inputType = temp.type.substring(10);
				//$("#dialogBackground").hide();
				$("input[name='" + inputType + "']").val("$" + temp.orgName + "$");
				$("input[name='id_" + inputType + "']").val(temp.variableId );
			}else if(temp.type == 'symbol'){
				$("#dialogBackground").hide();
				$("#content").textbox().insertText(temp.orgName);
			}else if(temp.type == 'action'){
				$("#dialogBackground").hide();
				$("#showAddAction").remove();
				var boardDiv = "<div style='border:1px solid #B8D0D6;padding:5px;margin:1px'>" + temp.id + "<input type='hidden' name = 'actionList' value='" + temp.orgName + "'>" +
						"<span style='float:right;'><a href='javascript:void(0)' onclick='deleteDiv(this)'>删除</a></span>" + "<\/div>"; 
				$("#actionShow").append(boardDiv);
			}else if(temp.type == 'actionUpdate'){
				$("#dialogBackground").hide();
				$("#showAddActionUpdate").remove();
				var boardDiv = "<div style='border:1px solid #B8D0D6;padding:5px;margin:1px'>" + temp.id + "<input type='hidden' name = 'actionList' value='" + temp.orgName + "'>" +
						"<span style='float:right;'><a href='javascript:void(0)' onclick='deleteDiv(this)'>删除</a></span>" + "<\/div>"; 
				$("#actionShowUpdate").append(boardDiv);
			}
		}
	});
